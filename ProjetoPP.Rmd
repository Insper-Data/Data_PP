...
base %>% 
  ...
  scale_fill_gradient(low = "seagreen1", high = "tomato2", limits = c(1, 4), breaks = seq(1, 4, 1)) +
   labs(title = "International Travel Controls",
        subtitle = ("In the beggining of September"),
        fill = "Index",
        caption = "Source: Oxford Policy Tracke") +
   theme_map() +
    theme(panel.background = element_rect(fill = "#b3d5e0"),
          legend.position = "right")
...
base %>% 
  ...
  geom_point() +
  geom_smooth(method = lm, se = FALSE) +
  geom_label_repel(data = subset(gen_names, `WJP Rule of Law Index: Overall Score`>0.82),
                   aes(label = country_name),
                   box.padding   = 0.35, 
                   point.padding = 0.5,
                   segment.color = 'grey50') 
...
overrallreg <- lm(`Overall Score` ~ `WJP Rule of Law Index: Overall Score` , data = base)
overrallreg
...
base %>% 
  select(country_name, iso_code, total_cases_per_million, `Factor 6: Regulatory Enforcement`, date) %>% 
  filter(date == "2020-09-10") %>% 
  filter(!is.na(`Factor 6: Regulatory Enforcement`), 
         !is.na(total_cases_per_million)) %>% 
  ggplot(mapping = aes(x = total_cases_per_million, y = `Factor 6: Regulatory Enforcement`)) +
  geom_point() +
  #geom_smooth(method = lm, se = FALSE) +
