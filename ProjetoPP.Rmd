...
# Cleaning the Democracy Index database (democracy)
...
iso <- iso_codes %>% 
  mutate(across(where(is.character), ~ str_replace_all(.x, "(\w+\s|\w+\s\w+)\\(", "\w+\s|\w+\s\w+)")))
...
base %>% 
  select(name, iso_code, days_after_1pm, date, region) %>% 
  group_by(name)%>% 
  slice(which.max(days_after_1pm)) %>% 
  ungroup() %>% 
  mutate(iso_code = fct_reorder(iso_code, desc(days_after_1pm))) %>% 
  mutate(days_after_1pm = days_after_1pm) %>% 
  ggplot(aes(iso_code, days_after_1pm)) +
  geom_col() +
  coord_flip() +
  theme(axis.text.y=element_blank(), axis.ticks.y=element_blank()) +
  labs(title = "Days distribution",
       x = "Every line is a country", 
       y = "Days after reachng 1 case per million")
