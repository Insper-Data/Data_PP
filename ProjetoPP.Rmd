...
#Arrumar os gráficos + Extender a análise


base %>% 
  select(country_name, iso_code, `Overall Score`, date) %>% view()
...
base %>% 
  ...
  ggplot() +
   ...
   scale_fill_gradient(low = "#FBC5D0", high = "#BE0E34", limits = c(1, 10), breaks = seq(1, 10, 1)) +
...
wjptidy %>% 
  ...
  ggplot(mapping = aes(x = `Factor 6: Regulatory Enforcement`, Factor, y = `Factor 1: Constraints on Government Powers`)) +
  geom_point() +
  geom_smooth(se = FALSE) +
#  geom_text(aes(label=ifelse(`Factor 6: Regulatory Enforcement`>0.7,as.character(iso_code),'')),hjust=0,vjust=0) +
...
wjptidy %>% 
  ...
  geom_point() +
  geom_smooth(se = FALSE) +
  #  geom_text(aes(label=ifelse(`7.3 Civil justice is free of corruption`>0.9,as.character(iso_code),'')),hjust=0,vjust=0) +
...
wjptidy %>% 
  ...
  ggplot(mapping = aes(x = `Factor 6: Regulatory Enforcement`, Factor, y = `Factor 5: Order and Security`)) +
  geom_point() +
  geom_smooth(se = FALSE) +
  #  geom_text(aes(label=ifelse(`7.3 Civil justice is free of corruption`>0.9,as.character(iso_code),'')),hjust=0,vjust=0) +
...
base %>% 
  ...
  filter(!is.na(`WJP Rule of Law Index: Overall Score`), 
         !is.na(`Overall Score`)) %>% 
  ggplot(mapping = aes(x = `WJP Rule of Law Index: Overall Score`, Factor, y = `Overall Score`)) +
  geom_point() +
  geom_smooth(se = FALSE) +
  geom_label_repel(data = subset(x, `WJP Rule of Law Index: Overall Score`>0.82),
                   aes(label = country_name),
                   box.padding   = 0.35, 
                   point.padding = 0.5,
                   segment.color = 'grey50') 
