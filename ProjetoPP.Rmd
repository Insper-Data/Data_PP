...
iso <- iso_codes %>% 
        ...
        #Colocar a Costa do Marfim (Cotê D'ivoire) // 
        #Bosnia // Congo // Suazilândia // Vietnam
...
base %>% 
  ...
  scale_fill_gradient(low = "seagreen1", high = "pink", limits = c(1, 4), breaks = seq(1, 4, 1)) +
...
base %>% 
  ...
  ggplot(mapping = aes(x = log(total_cases_per_million), y = `Factor 6: Regulatory Enforcement`)) +
  geom_point() +
  #geom_smooth(method = lm, se = FALSE) +
