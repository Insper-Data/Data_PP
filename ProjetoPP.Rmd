...
# setwd("C:/Users/Bernardo Lisboa/Desktop/SEMESTRES INSPER/Entidades e Challenges/Insper Data/Data_PP/bases")

setwd("C:/Users/User/Desktop/dataset/Data_PP/bases")

# setwd("C:/Users/Bruno Bregola/Desktop/Projeto/Data_PP/bases")
...
df2 <- t %>%
    filter(country %in% c("United States", "Japan", "Argentina") & escala == "tests performed")
...
```{r}
# não sei resolver talvez fazer um mapa
...
base %>% 
  ...
  ggplot() +
   geom_sf(aes(geometry = geom), data = world, fill = "lightgrey") +
   geom_sf(aes(geometry = geom, fill = type.x, alpha = 0.8)) +
...
max(cpt$e1_income_support, na.rm = TRUE)
max(cpt$c5_close_public_transport, na.rm = TRUE)
...
cpt %>%
  ...
  ggplot(aes(x = media_fechamento_transporte, y = media_suportederenda)) +
    geom_point() +
    geom_smooth(method = lm, se = FALSE) +
  labs(title = "Transport closing versus income support",
       x = "Mean of public transport closing",
       y = "Mean of Income Support") + 
  xlim(0,2) +
  ylim(0,2) +
  theme_classic()
...
min(cpt$e4_international_support, na.rm = TRUE)
min(cpt$c8_international_travel_controls, na.rm = TRUE)
...
base %>%
  ...
  xlim(0, 4) +
  ylim(0, 30) +
   labs(title = "Impact of travel controls on levels of international support",
        subtitle = "Colors represent the types of democracy",
...
```{r}
base %>%
  select(name, iso_code, iso2, confirmed_cases, days_after_1pm) %>%
  filter(!is.na(confirmed_cases), days_after_1pm == 250) %>%
  clean_names() %>%
  left_join(world, by = c("iso2" = "iso_a2")) %>%
  ggplot() +
   geom_sf(aes(geometry = geom), data = world, fill = "lightgrey") +
   geom_sf(aes(geometry = geom, fill = confirmed_cases)) +
   labs(title = "Confirmed COVID-19 cases",
        subtitle = ("Values for 250 days after 1 case per million habitants"),
        fill = "Cases",
        caption = "Source: Our World In Data Covid") +
   scale_fill_gradient(low = "seagreen", high = "coral") +
   theme_map() +
    theme(panel.background = element_rect(fill = "#b3d5e0"),
...
```{r}
base %>%
  select(name, iso_code, iso2, days_after_1pm, gini_index_world_bank_estimate) %>%
  filter(!is.na(gini_index_world_bank_estimate), days_after_1pm == 250) %>%
  clean_names() %>%
  left_join(world, by = c("iso2" = "iso_a2")) %>%
  ggplot() +
   geom_sf(aes(geometry = geom), data = world, fill = "lightgrey") +
   geom_sf(aes(geometry = geom, fill = gini_index_world_bank_estimate)) +
   labs(title = "Gini Index",
        subtitle = "latest data for each country",
        fill = "Index",
        caption = "Source: World Bank") +
   scale_fill_gradient(low = "springgreen3", high = "tomato2") +
   theme_map() +
    theme(panel.background = element_rect(fill = "#b3d5e0"),
            legend.position = "bottom")
...
base %>% 
  select(name, iso_code, `Cumulative total per thousand`, total_cases_per_million, days_after_1pm,
         `Income Group`, region) %>% 
  ...
  geom_path(aes(y = logct, x = logcasepthous, color = `Income Group`)) +
  geom_smooth(aes(y = logct, x = logcasepthous), method = lm, se = FALSE) +
  geom_abline(slope = 0.45, intercept = 0) + 
  theme_clean() +
  xlim(0, 20) +
  scale_alpha(guide = 'none') +
  facet_wrap(~ `Income Group`) +
  labs(title = "Testing versus Cases",
       x = "Log tests",
       y = "Log cases per thousand")
...
base %>% 
  ...
  theme_clean() +
  labs(title = "Daily testing per thousand",
       subtitle = "by continents, after 1 case per million",
       x = "Days",
       y = "Mean number of tests",
       color = element_blank())
...
base %>% 
  ...
  theme_clean() +
  labs(title = "Daily testing per thousand",
       subtitle = "by type of regime, after 1 case per million",
       x = "Days",
       y = "Mean number of tests",
       color = element_blank())
...
## World Justice Project Rule of Law 2019
...
base %>% 
  ...
  geom_smooth(method = lm, se = FALSE) +
  geom_label_repel(data = subset(wjptidy, name == c("Brazil", "China", "United States of America")),
                   aes(label = name),
                   box.padding   = 0.35, 
                   point.padding = 0.5,
  ...
  xlim(0, 1) +
  ylim(0, 1) +
  labs(title = "Regulatory Enforcement and Constrantis on Government Powers",
       subtitle = "by type of democracy",
       color = element_blank())
...
base %>% 
  ...
  xlim(0, 1) +
  ylim(0, 1) +
  labs(title = "Civil Justice is free of discrimination and Absence of Corruption",
       subtitle = "by type of democracy",
       color = element_blank())
...
base %>% 
  select(name, iso_code, `Factor 5: Order and Security`, `Factor 6: Regulatory Enforcement`, type) %>% 
  filter(!is.na(type)) %>% 
  ...
  xlim(0, 1) +
  ylim(0, 1) +
  labs(title = "Order and Security and Regulatory Enforcement",
        color = element_blank())
...
x <- base %>% 
  filter(!is.na(`WJP Rule of Law Index: Overall Score`), 
         !is.na(`Overall Score`), days_after_1pm == 60)

base %>% 
  select(name, iso_code, `WJP Rule of Law Index: Overall Score`, `Overall Score`, days_after_1pm, `Income Group`) %>% 
  filter(days_after_1pm == 60) %>% 
  filter(!is.na(`WJP Rule of Law Index: Overall Score`), 
         !is.na(`Overall Score`)) %>% 
  ...
  geom_smooth(method = lm, se = FALSE) +
  geom_label_repel(data = subset(x, name == c("United States of America", "China")),
                   aes(label = name),
                   box.padding   = 0.35, 
                   point.padding = 0.5,
                   segment.color = 'grey50') +
...
base %>% 
  select(name, iso_code, `3.3 Civic participation`, `Overall Score`, days_after_1pm, `Income Group`) %>% 
  ...
  geom_point(aes(color = `Income Group`)) +
  geom_smooth(method = lm, se = FALSE) +
  geom_label_repel(data = subset(gen_names_2, name == c("Brazil", "China")),
                   aes(label = name),
                   box.padding   = 0.35, 
                   point.padding = 0.5,
                   segment.color = 'grey50') +
...
base %>% 
  select(name, iso_code, `Factor 2: Absence of Corruption`, `Overall Score`, days_after_1pm, `Income Group`) %>% 
  filter(days_after_1pm == 30) %>% 
  filter(!is.na(`Factor 2: Absence of Corruption`), 
         !is.na(`Overall Score`)) %>% 
  ...
  geom_point(aes(color = `Income Group`)) +
  geom_smooth(method = lm, se = FALSE) +
  geom_label_repel(data = subset(gen_names_3, name == c("Brazil", "China")),
                   aes(label = name),
                   box.padding   = 0.35, 
                   point.padding = 0.5,
                   segment.color = 'grey50') +
...
base %>% 
  select(name, iso_code, total_cases_per_million, `Factor 6: Regulatory Enforcement`,
         days_after_1pm, type) %>% 
  filter(!is.na(`Factor 6: Regulatory Enforcement`), 
         !is.na(total_cases_per_million), !is.na(type), days_after_1pm == 100) %>% 
...
base %>% 
  ...
  filter(!is.na(`Factor 4: Fundamental Rights`), 
         !is.na(c6_stay_at_home_requirements),!is.na(days_after_1pm), days_after_1pm == 1) %>% 
  ggplot(aes(x = c6_stay_at_home_requirements, y = `Factor 4: Fundamental Rights`)) +
  geom_histogram(aes(y = stat(count/sum(count))), fill = "steelblue3", alpha = 0.7) +
  geom_jitter() +
  scale_alpha(guide = 'none') +
  theme_classic() +
  ylim(0, 1) +
  labs(title = "Fundamental Rights versus Stay at home requirements",
       subtitle = "at 1 day after 1 case per million habitants",
    x = "Stay at home requirements")
...
base %>% 
  select(name, iso_code, `Factor 4: Fundamental Rights`, c6_stay_at_home_requirements, days_after_1pm) %>% 
  filter(!is.na(`Factor 4: Fundamental Rights`), 
         !is.na(c6_stay_at_home_requirements),!is.na(days_after_1pm), days_after_1pm == 40) %>% 
  ggplot(aes(x = c6_stay_at_home_requirements, y = `Factor 4: Fundamental Rights`)) +
  geom_histogram(aes(y = stat(count/sum(count))), fill = "steelblue3", alpha = 0.7) +
  geom_jitter() +
  scale_alpha(guide = 'none') +
  ...
  ylim(0, 1) +
  labs(title = "Fundamental Rights versus Stay at home requirements",
       subtitle = "at 40 days after 1 case per million habitants",
    x = "Stay at home requirements")
...
base %>% 
  select(name, iso_code, `Factor 4: Fundamental Rights`, c6_stay_at_home_requirements, days_after_1pm) %>% 
  filter(!is.na(`Factor 4: Fundamental Rights`), 
         !is.na(c6_stay_at_home_requirements),!is.na(days_after_1pm), days_after_1pm == 75) %>% 
  ggplot(aes(x = c6_stay_at_home_requirements, y = `Factor 4: Fundamental Rights`)) +
  geom_histogram(aes(y = stat(count/sum(count))), fill = "steelblue3", alpha = 0.7) +
  geom_jitter() +
  scale_alpha(guide = 'none') +
  ...
  ylim(0, 1) +
  labs(title = "Fundamental Rights versus Stay at home requirements",
       subtitle = "at 75 days after 1 case per million habitants",
    x = "Stay at home requirements")
...
base %>% 
  select(name, iso_code, `Factor 4: Fundamental Rights`, c6_stay_at_home_requirements, days_after_1pm) %>% 
  filter(!is.na(`Factor 4: Fundamental Rights`), 
         !is.na(c6_stay_at_home_requirements),!is.na(days_after_1pm), days_after_1pm == 150) %>% 
  ggplot(aes(x = c6_stay_at_home_requirements, y = `Factor 4: Fundamental Rights`)) +
  geom_histogram(aes(y = stat(count/sum(count))), fill = "steelblue3", alpha = 0.7) +
  geom_jitter() +
  scale_alpha(guide = 'none') +
  ...
  ylim(0, 1) +
  labs(title = "Fundamental Rights versus Stay at home requirements",
       subtitle = "at 150 days after 1 case per million habitants",
    x = "Stay at home requirements")
...
base %>% 
  select(name, iso_code, `Factor 4: Fundamental Rights`, c6_stay_at_home_requirements, days_after_1pm) %>% 
  filter(!is.na(`Factor 4: Fundamental Rights`), 
         !is.na(c6_stay_at_home_requirements),!is.na(days_after_1pm), days_after_1pm == 225) %>% 
  ggplot(aes(x = c6_stay_at_home_requirements, y = `Factor 4: Fundamental Rights`)) +
  geom_histogram(aes(y = stat(count/sum(count))), fill = "steelblue3", alpha = 0.7) +
  geom_jitter() +
  scale_alpha(guide = 'none') +
  ...
  ylim(0, 1) +
  labs(title = "Fundamental Rights versus Stay at home requirements",
       subtitle = "at 225 days after 1 case per million habitants",
    x = "Stay at home requirements")
...
mapcovid <- base %>% 
  ...
  ggplot() +
   ...
   scale_fill_gradient(high = "tomato3", low = "turquoise3", limits = c(0, 1000), breaks = seq(0, 1000, 100)) +
...
base %>% 
  ...
  geom_histogram() +
  theme_classic() +
  labs(title = "Distribution of days after 1 case per million habitants",
       y = "# of countries",
       x = "Days")
...
base %>% 
  ...
  clean_names() %>% 
  ggplot(aes(gini_index_world_bank_estimate, total_cases_per_million)) +
  geom_point(aes(size = 2.5, alpha = 0.5, color = region)) +
  scale_alpha(guide = 'none') +
  guides(size = FALSE) +
  geom_smooth(method = lm, se = FALSE) +
...
colunas <- tibble(colnames(base))

x <- binsreg(log(b$`Cumulative total per thousand`), b$gini_index_world_bank_estimate, w = b$total_cases_per_million, nbins = 20)

summary(x)

#The next two regressions were generated without the use of control variables because of an error that occurred the last time we tried to run the code but we believe it is a punctual problem that can be solved.

x2 <- binsreg(log(b$total_cases_per_million), b$`Overall Score`, nbins = 20, polyreg = TRUE)

summary(x2)

x3 <- binsreg(b$`Overall Score`, b$`WJP Rule of Law Index: Overall Score`, nbins = 20, polyreg = TRUE)

summary(x3)

x4 <- binsreg(log(b$total_cases_per_million), b$gini_index_world_bank_estimate, w = log(b$`Cumulative total per thousand`), nbins = 20)
