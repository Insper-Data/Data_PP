...
contador = round(as.numeric(difftime(Sys.Date(), "2020-09-13", units = "days"))) 
...
# junto %>% filter(is.na(name)) %>% view ### Taking Kosovo out 


# Getting all data together

base_semi <- left_join(junto, wjp_cpt, by = c("iso_code", "region", "iso2", "date", "name"))


base_semi %>% count(name) 

## anti_join(junto, wjp_cpt,  by = c("iso_code", "region", "iso2", "date", "name")) %>% view #### Excluding the 31/12/2019 from the database
...
p <- ggplot(dmi, aes(x=as.factor(id), y=`Overall Score`, fill = type)) +    
  ...
  coord_polar() + 
  geom_text(data = label_data, aes(x = id, y = `Overall Score` + 1, label = name, hjust = hjust), color = "black", fontface = "bold",alpha=0.6,
...
## CANADA DOES NOT HAVE INFORMATION - PROBLEM WITH MISSING DATA ABOUT THE COUNTRY

cpt %>% 
  select(name, c1_flag: h6_flag, iso_code, iso2) %>% 
  mutate(across(c1_flag: h6_flag, ~ as.numeric(.x))) %>% 
  filter(across(everything(), ~ .x >= 0)) %>% view()
  clean_names() %>% 
  group_by(name) %>% 
  summarise(generalismo = mean((c1_flag + 
         c2_flag + c3_flag + c4_flag + c5_flag +
         c6_flag + c7_flag + e1_flag + h1_flag)/8), iso2 = iso2) %>%  
  ungroup() %>% 
  view()
...
1) Group countries according to their income group and analyze, in general, the average score of the WJP Index, aiming to obtain some relation between the product level and the local institutional strength
```{r}
...
2) Group countries according to the region and analyze, in general, the average score of the WJP Index, aiming to obtain some relation between the local institutional strength and the regions of the map
```{r}
...
4) Estudar a relação entre as médias do Fator 1 (Constraints on Government Powers) e do Fator 6 (Regulatory Enforcement)
```{r}
...
5) Construir um mapa com a média do fator 6
...
6)Estudar a relação entre a variável 7.3 (Civil Justice is free of discrimination) e o Fator 2 (Absence of Corruption)
```{r}
...
7) Estudar a relação entre as médias do Fator 5 (Order and Security) e do Fator 6 (Regulatory Enforcement)
```{r}
...
8) Relação entre o Overall Score do WJP e o Democracy Index, da The Economist
```{r}
...
9)Relação entre a variável 3.3 (Civil Participation) do WJP e o Democracy Index, da The Economist
...
10)Relação entre o fator 2 (Absence of Corruption) do WJP e o Democracy Index, da The Economist
...
11)Relação entre a média do Fator 6 e as taxas populacionais de falecimento e de casos totais por COVID-19
...
12)Relação entre o Fator 4 (Fundamental Rights) e a variável C6 (Stay at home requirements) do Oxford Response Tracker
...
12B)Relação animada conforme dias após o marco 1 morte por milhão
...
13) Comparar o mapa com a média do Fator 6 com o mapa com dados da Covid-19, controlando os dados pelo tamanho populacional
...
base %>% filter(days_after_1pm == 1) %>% 
  arrange(population) %>% 
  ggplot(aes(name, population)) +
  geom_col()

base %>%
  ggplot(aes(x = reorder(iso_code, population), population)) + 
  geom_col() +
  coord_flip() +
  theme_clean()
...
reg1 <- lm_robust(total_cases_per_million ~ ., data = subset(b, select=c(-name, -iso_code, -date, -iso2, -confirmed_cases, -total_cases, -new_cases, -total_deaths, -new_deaths, -new_cases_per_million, -confirmed_deaths, -total_deaths_per_million, -`Short-term tests per case`, -`Short-term positive rate`, -`Cumulative total per thousand`, -`7-day smoothed daily change`, -`7-day smoothed daily change per thousand`, -`Daily change in cumulative total per thousand`, -new_deaths_smoothed_per_million, -new_deaths_per_million, -new_cases_smoothed_per_million, -new_deaths_smoothed, -new_cases_smoothed, -`Daily change in cumulative total`)))
...
reg2 <- lm(total_cases_per_million ~ `Overall Score` +`WJP Rule of Law Index: Overall Score` + gini_index_world_bank_estimate + population + gdp_per_capita + aged_65_older + median_age + population_density + Region + `Income Group` + days_after_1pm + extreme_poverty + human_development_index, data = b)
