...
base %>% 
  ...
  ggplot() +
   ...
   labs(title = "Confirmed COVID-19 cases",
        subtitle = ("Values for 250 days after 1 death per milion habitants"),
        fill = "Cases",
        caption = "Source: Our World In Data Covid") +
   scale_fill_gradient(low = "seagreen", high = "coral", breaks = c(2.5, 5, 7.5, 10)) +
   theme_map() +
    theme(panel.background = element_rect(fill = "#b3d5e0"),
            legend.position = "right") 
...
p11 <- base %>% 
  select(name, iso_code, `Factor 4: Fundamental Rights`, c6_stay_at_home_requirements, days_after_1pm) %>% 
  filter(!is.na(`Factor 4: Fundamental Rights`), 
         !is.na(c6_stay_at_home_requirements),!is.na(days_after_1pm), days_after_1pm < 250) %>% 
  ggplot(aes(x = c6_stay_at_home_requirements, y = `Factor 4: Fundamental Rights`)) +
  geom_density(aes(x =..density.., y = `Factor 4: Fundamental Rights`), fill = "forestgreen", alpha = 0.5) +
  geom_density(aes(y = ..density..), fill = "steelblue3", alpha = 0.7) +
  geom_jitter() +
  scale_alpha(guide = 'none') +
  theme_classic() +
  ylim(0, 1.25) +
