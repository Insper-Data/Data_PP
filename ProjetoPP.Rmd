...
base %>% 
  ...
  filter(!is.na(total_cases_per_million), 
         !is.na(`Cumulative total per thousand`)) %>% 
  filter(days_after_1pm == 200) %>% 
  mutate(logcasepthous = log(total_cases_per_million*1000)) %>% 
  ggplot(x = `Cumulative total per thousand`, y = logcasepthous) +
  geom_point(aes(x = `Cumulative total per thousand`, y = logcasepthous)) +
  theme_clean() +
  labs(title = "Cumulative testing per thousand and log(Total cases per thousand)")
...
base %>% 
  ...
  geom_path(aes(x = days_after_1pm, y = logmeantest)) +
  theme_clean() +
