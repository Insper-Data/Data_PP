...
cpt <- CRT %>% 
  ...
  filter(iso_code != "RKS") # tiramos o Kosovo pois ele não está nas outra bases e complicaria a análise. 
                            # Acreditamos que ele tenha pouca representatividade
...
# wjp_cpt %>% filter(is.na(name)) %>% view #### Kosovo não aparece pq não está em nenhuma base alem do CRT
...
# junto %>% filter(is.na(name)) %>% view ### tiramos o Kosovo 
...
## anti_join(junto, wjp_cpt,  by = c("iso_code", "region", "iso2", "date", "name")) %>% view #### excluimos o dia 31/12/2019
...
#CANADÁ SEM DADO - PROBLEMA COM NA
...
3) Construir um mapa com Overall Score (WJP)
...
1) Total de cases per million x Gini Index
...
# Do the regressions structured in the Doc File and use PCA as an alternative analysis,
